...
class PaypalField extends \ElementorPro\Modules\Forms\Fields\Field_Base
{
    ...
    public function run_once()
    {
        ...
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_item_name');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_item_value');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_item_sku');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_item_description');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_disable_validation');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_value_from_field');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_value_field_id');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_name_from_field');
        $save_guard->register_unsafe_control('form', 'form_fields::dce_form_paypal_name_field_id');
    }
    ...
    public function process_field($field, Classes\Form_Record $record, Classes\Ajax_Handler $ajax_handler)
    {
        ...
        $order_context = \implode(':', [(string) get_current_blog_id(), (string) $record->get_form_settings('id'), (string) $id]);
        if (isset(self::$validated_orders[$order_id])) {
            if (!\is_string(self::$validated_orders[$order_id]) || !\hash_equals(self::$validated_orders[$order_id], $order_context)) {
                $ajax_handler->add_error($id, $error_msg);
            }
            return;
        }
        ...
        self::$validated_orders[$order_id] = $order_context;
    }
    /**
