...
class Counter extends \ElementorPro\Modules\Forms\Fields\Field_Base
{
    ...
    private function is_valid_ajax_post_id(int $post_id) : bool
    {
        return $post_id > 0 && get_post($post_id) instanceof \WP_Post && Helper::can_current_user_view_post($post_id);
    }
    /** @param array<string,mixed> $settings */
    private function get_ajax_post_id(array $settings) : ?int
    {
        $form_post_id = isset($_POST['post_id']) && \is_scalar($_POST['post_id']) ? absint(wp_unslash((string) $_POST['post_id'])) : 0;
        if (!$this->is_valid_ajax_post_id($form_post_id)) {
            return null;
        }
        if (($settings['dce_counter_unique'] ?? 'yes') === 'yes' || !isset($_POST['queried_id'])) {
            return $form_post_id;
        }
        if (!\is_scalar($_POST['queried_id'])) {
            return null;
        }
        $queried_id = absint(wp_unslash((string) $_POST['queried_id']));
        if (!$queried_id) {
            return $form_post_id;
        }
        return $this->is_valid_ajax_post_id($queried_id) ? $queried_id : null;
    }
    public function get_post_id_and_meta($form_id, $settings, $from)
    {
        if ($settings['dce_counter_use_other'] === 'yes') {
            ...
            if ($from === 'render') {
                ...
                $post_id = $this->get_ajax_post_id($settings);
            }
        }
        return [$post_id, $meta];
    }
    ...
    public function validation($field, Form_Record $record, Ajax_Handler $ajax_handler)
    {
        ...
        if (null === $post_id) {
            $ajax_handler->add_error($field['id'], esc_html__('Invalid counter target.', 'dynamic-content-for-elementor'));
            return;
        }
        $value = get_post_meta($post_id, $meta, \true);
        if ($value === \false || $value === '') {
    ...
    public function update_counters($record, $ajax_handler)
    {
        ...
        foreach ($fields as $field) {
            ...
            if (null === $post_id) {
                $ajax_handler->add_error($field['custom_id'], esc_html__('Invalid counter target.', 'dynamic-content-for-elementor'));
                return;
            }
