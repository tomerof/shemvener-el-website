...
class Context extends \DynamicContentForElementor\Extensions\DynamicVisibility\Triggers\Base
{
    ...
    {
        $element->add_control('dce_visibility_parameter', ['label' => esc_html__('Parameter', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'description' => esc_html__('Type here the name of the parameter passed in GET, COOKIE or POST method', 'dynamic-content-for-elementor'), 'ai' => ['active' => \false]]);
        $element->add_control('dce_visibility_parameter_method', ['label' => esc_html__('Parameter Method', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => ['GET' => 'GET', 'POST' => 'POST', 'REQUEST' => 'REQUEST', 'COOKIE' => 'COOKIE', 'SERVER' => 'SERVER'], 'default' => 'REQUEST', 'description' => esc_html__('Warning: with the SERVER method, HTTP_* keys come from request headers sent by the visitor and can be faked. Use this as a non-security hint only, never to protect sensitive content.', 'dynamic-content-for-elementor'), 'condition' => ['dce_visibility_parameter!' => '']]);
        $element->add_control('dce_visibility_parameter_status', ['label' => esc_html__('Parameter Status', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => Helper::get_compare_options(), 'default' => 'isset', 'condition' => ['dce_visibility_parameter!' => '']]);
        $element->add_control('dce_visibility_parameter_value', ['label' => esc_html__('Parameter Value', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'description' => esc_html__('The specific value of the parameter', 'dynamic-content-for-elementor'), 'ai' => ['active' => \false], 'condition' => ['dce_visibility_parameter!' => '', 'dce_visibility_parameter_status!' => ['not', 'isset']]]);
    ...
    public function check_conditions($settings, &$triggers, &$conditions, &$required, $element)
    {
        if (isset($settings['dce_visibility_parameter']) && $settings['dce_visibility_parameter']) {
            ...
            $required['dce_visibility_parameter'] = \true;
            $my_val = null;
        ...
        if (!empty($settings['dce_visibility_lang'])) {
            ...
            $required['dce_visibility_lang'] = \true;
        ...
        if (!empty($settings['dce_visibility_max_day'])) {
            ...
            $required['dce_visibility_max_day'] = \true;
        ...
        if (!empty($settings['dce_visibility_max_total'])) {
            ...
            $required['dce_visibility_max_total'] = \true;
            $dce_visibility_max = get_option('dce_visibility_max', []);
        ...
        if (!empty($settings['dce_visibility_conditional_tags_site']) && \is_array($settings['dce_visibility_conditional_tags_site'])) {
            $required['dce_visibility_conditional_tags_site'] = \true;
