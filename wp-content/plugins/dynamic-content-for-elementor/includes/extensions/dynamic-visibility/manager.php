    ...
    public function is_hidden($element)
    {
        ...
        if (!empty($settings['dce_visibility_triggers'])) {
            ...
            if (\count($settings['dce_visibility_triggers']) === 1 && \in_array('events', $settings['dce_visibility_triggers'], \true)) {
                ...
                if ($hidden) {
                    self::$hidden_elements[$element->get_id()] = ['mode' => 'show', 'operator' => 'or', 'conditions' => ['events' => __('Events (client-side)', 'dynamic-content-for-elementor')], 'events' => ['click' => $settings['dce_visibility_click'] ?? '', 'load' => !empty($settings['dce_visibility_load'])]];
                }
                return $hidden;
            }
        ...
        if ($hidden) {
            ...
            self::$hidden_elements[$element->get_id()] = ['mode' => $display_mode_is_show ? 'show' : 'hide', 'operator' => $logical_connective, 'conditions' => $triggers];
        }
        return $hidden;
    }
    /**
