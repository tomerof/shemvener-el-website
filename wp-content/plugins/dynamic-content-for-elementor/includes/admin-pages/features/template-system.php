...
class TemplateSystem
{
    ...
    public function render()
    {
        ...
        foreach ($typesRegistered as $chiave) {
            $preview[$chiave] = (string) get_post_type_archive_link($chiave);
            if ($chiave == 'page') {
                $preview[$chiave] = get_home_url();
                $id_privacy = get_option('wp_page_for_privacy_policy');
                if ($id_privacy) {
                    $preview[$chiave] = (string) get_permalink($id_privacy);
                }
            }
            if ($chiave == 'post') {
                $page_for_post = get_option('page_for_posts');
                if ($page_for_post) {
                    $preview[$chiave] = (string) get_permalink($page_for_post);
                }
            }
        ...
        foreach ($customTaxonomies as $chiave) {
            $terms = get_terms(['taxonomy' => $chiave, 'number' => 1, 'update_term_meta_cache' => \false]);
            if (!is_wp_error($terms) && !empty($terms)) {
                $term_link = get_term_link(\reset($terms));
                $preview[$chiave] = is_wp_error($term_link) ? get_home_url() : $term_link;
            } else {
                $preview[$chiave] = get_home_url();
            }
        ...
        esc_html_e('Enable', 'dynamic-content-for-elementor');
        ?></b></span>
								</label>
        ...
        esc_html_e('Disable', 'dynamic-content-for-elementor');
        ?></b></span>
								</label>
								<br><br>
        ...
        foreach ($dceTemplate as $tkey => $tvalue) {
            ...
            foreach ($tvalue['options'] as $chiave => $label_t) {
                ...
                if ($dce_template_used_archive) {
                    ...
                    echo esc_attr($dce_tkey . '-archive');
                ...
                ?>
												<abbr title="<?php 
                echo esc_attr($chiave);
                ?>"><?php 
                echo esc_html($label_t);
                ?></abbr>
															</a>
														</li>
														<?php 
        ...
        foreach ($dceTemplate as $tkey => $tvalue) {
            foreach ($tvalue['options'] as $chiave => $label_t) {
                ...
                foreach ($tvalue['templates'][$chiave] as $skey => $svalue) {
                    ...
                    ?>">
										<a class="nav-tab-link" href="#<?php 
                    echo esc_attr($tkey . '-' . $chiave . '-' . $skey);
                    ?>" onClick="
																	jQuery('#<?php 
                    echo esc_js($tkey . '-' . $chiave);
                ...
                esc_attr_e('Save Settings', 'dynamic-content-for-elementor');
                ?>">
												</div>
											</div>
										</div>
                ...
                foreach ($tvalue['templates'][$chiave] as $skey => $svalue) {
                    ...
                    if ($skey != 'single') {
                        ...
                        ?>"></div>
								...
								<h4><?php 
                        esc_html_e('Before', 'dynamic-content-for-elementor');
                        ?></h4>
																	<?php 
                    ...
                    if ($skey == 'single') {
                        ?>
								<h4><?php 
                        esc_html_e('Page Template', 'dynamic-content-for-elementor');
                        ?></h4>
																		<?php 
                        $this->select_template($dce_key, $templates);
                        ?>
                        ...
                        ?>
									...
									<h4><?php 
                        esc_html_e('Layout', 'dynamic-content-for-elementor');
                        ?></h4>
                        ...
                        esc_html_e('Default', 'dynamic-content-for-elementor');
                        ?></span>
																			</label>
																			<label class="dce-radio-container dce-radio-container-template">
                        ...
                        ?>]">
																				<span class="dce-radio-checkmark"></span>
										<span class="dce-radio-label"><?php 
                        esc_html_e('Full-Width', 'dynamic-content-for-elementor');
                        ?></span>
																			</label>
																			<label class="dce-radio-container dce-radio-container-template">
                        ...
                        ?>]">
																				<span class="dce-radio-checkmark"></span>
										<span class="dce-radio-label"><?php 
                        esc_html_e('Canvas', 'dynamic-content-for-elementor');
                        ?></span>
																			</label>
																		</div>
																		<?php 
                    ...
                    if ($skey == 'archive') {
                        ?>
								<h4><?php 
                        esc_html_e('Template', 'dynamic-content-for-elementor');
                        ?></h4>
																		<?php 
                        $this->select_template($dce_key, $templates);
                        ?>
                        ...
                        ?>]">
																				<span class="dce-radio-checkmark"></span>
										<span class="dce-radio-label"><?php 
                        esc_html_e('Boxed', 'dynamic-content-for-elementor');
                        ?></span>
																			</label><label class="dce-radio-container dce-radio-container-template">
                    ...
                    if ($skey != 'single') {
                        ...
                        ?>"></div>
							...
							<h4><?php 
                        esc_html_e('After', 'dynamic-content-for-elementor');
                        ?></h4>
																		<?php 
                        $this->select_template($dce_key, $templates);
                        ?>
    ...
    private function archive_settings($dce_key)
    {
        ...
        echo esc_attr($dce_key);
        ?>"><?php 
        esc_html_e('Columns', 'dynamic-content-for-elementor');
        ?></label>
