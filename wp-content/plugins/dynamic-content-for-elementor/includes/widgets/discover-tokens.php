...
class DiscoverTokens extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    protected function safe_render()
    {
        ...
        if (!Tokens::is_active()) {
            Helper::notice(\false, \sprintf(
                /* translators: %s: plugin name */
                esc_html__('Tokens are not active. You can activate it from WP Dashboard > %s > Settings > Tokens', 'dynamic-content-for-elementor'),
                DCE_PRODUCT_NAME
            ));
            return;
        }
    ...
    protected function show_list(string $type)
    {
        ...
        if (!\in_array($realtype, get_option('dce_active_tokens', $active_tokens), \true)) {
            \printf(
                /* translators: %s: plugin name */
                esc_html__('This token type is not active. If you want to use it you can activate it from WP Dashboard > %s > Settings > Tokens > Active Tokens', 'dynamic-content-for-elementor'),
                DCE_PRODUCT_NAME
            );
            return;
        }
    ...
    protected function check_plugin_dependencies(string $type)
    {
        ...
        if (!empty($plugin_depends) && !Helper::is_dependencies_satisfied($plugin_depends)) {
            $plugins = Helper::to_readable_string(Helper::is_dependencies_satisfied($plugin_depends));
