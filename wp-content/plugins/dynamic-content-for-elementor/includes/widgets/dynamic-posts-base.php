...
class DynamicPostsBase extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    public function run_once()
    {
        ...
        $save_guard->register_unsafe_control('any', 'list_items::item_token_code');
    }
...
, 'tokens' => '<a href="[post:permalink]">[post:thumb]</a><h4><a href="[post:permalink]">[post:title|esc_html]</a></h4><p>[post:excerpt]</p><a class="btn btn-primary" href="[post:permalink]">' . esc_html__('Read more', 'dynamic-content-for-elementor') . '</a>']), 'ai' => ['active' => \false], 'dynamic' => ['active' => \false], 'description' => \DynamicContentForElementor\Plugin::instance()->text_templates->get_default_value(['dynamic-shortcodes' => esc_html__('You can use Dynamic Shortcodes and HTML.', 'dynamic-content-for-elementor'), 'tokens' => esc_html__('You can use HTML and Tokens.', 'dynamic-content-for-elementor')]), 'condition' => ['_skin' => ['', 'grid', 'carousel', 'grid-filters', 'dualcarousel', 'smoothscroll', '3d', 'accordion'], 'style_items' => 'html_tokens']]);
        ...
        if (\DynamicContentForElementor\Plugin::instance()->template_system->is_active()) {
            $this->add_control('native_templatemode_enable', ['label' => esc_html__('Template System Block', 'dynamic-content-for-elementor'), 'description' => \sprintf(
                /* translators: %s: plugin name */
                esc_html__('Use the template associated with the type (Menu: %s > Features > Template System) to manage the appearance of the individual elements of the grid ', 'dynamic-content-for-elementor'),
                DCE_PRODUCT_NAME
            ), 'type' => Controls_Manager::SWITCHER, 'default' => '', 'render_type' => 'template', 'condition' => ['_skin' => ['', 'grid', 'carousel', 'grid-filters', 'dualcarousel', 'smoothscroll', '3d', 'accordion'], 'style_items' => 'template', 'templatemode_enable_2' => '']]);
        }
        ...
        $this->add_control('post_wrapper_tag', ['label' => esc_html__('Post Wrapper Tag', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => ['article' => 'article', 'section' => 'section', 'div' => 'div', 'aside' => 'aside'], 'default' => 'article', 'separator' => 'before', 'description' => esc_html__('Choose the HTML tag to wrap each post item.', 'dynamic-content-for-elementor'), 'render_type' => 'template', 'conditions' => ['terms' => [['name' => '_skin', 'operator' => '!in', 'value' => ['list', 'table', 'accordion']]]]]);
    ...
    protected function register_query_controls()
    {
        ...
        $this->add_control('post_type', ['label' => esc_html__('Post Type', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT2, 'options' => Helper::get_public_post_types(), 'multiple' => \true, 'label_block' => \true, 'default' => [], 'condition' => ['query_type' => ['get_cpt', 'search_page', 'sticky_posts', 'relationship']]]);
        ...
        $this->add_control('querydate_field', ['label' => esc_html__('Date Field', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::SELECT, 'options' => ['post_date' => esc_html__('Publish Date', 'dynamic-content-for-elementor'), 'post_modified' => esc_html__('Modified Date', 'dynamic-content-for-elementor'), 'post_meta' => esc_html__('Post Meta', 'dynamic-content-for-elementor')], 'default' => 'post_date', 'condition' => ['query_filter' => 'date', 'querydate_mode!' => ['', 'future']]]);
        ...
        $this->add_control('querydate_field_meta', ['label' => esc_html__('Meta Field', 'dynamic-content-for-elementor'), 'type' => 'ooo_query', 'placeholder' => esc_html__('Meta key or Name', 'dynamic-content-for-elementor'), 'label_block' => \true, 'query_type' => 'metas', 'object_type' => 'post', 'description' => esc_html__('Select a date meta field.', 'dynamic-content-for-elementor'), 'separator' => 'before', 'dynamic' => ['active' => \false], 'condition' => ['query_filter' => 'date', 'querydate_mode!' => 'future', 'querydate_field' => 'post_meta']]);
        $this->add_control('querydate_field_meta_format', ['label' => esc_html__('Meta Date Format', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'placeholder' => esc_html__('Ymd, Y-m-d, Y-m-d H:i:s, etc.', 'dynamic-content-for-elementor'), 'label_block' => \true, 'default' => '', 'description' => esc_html__('PHP date format.', 'dynamic-content-for-elementor'), 'condition' => ['query_filter' => 'date', 'querydate_mode!' => 'future', 'querydate_field' => 'post_meta']]);
        $this->add_control('querydate_field_meta_format_autodetect', ['type' => Controls_Manager::BUTTON, 'text' => esc_html__('Auto-Detect Date Format', 'dynamic-content-for-elementor'), 'button_type' => 'default', 'event' => 'dce:detectDateFormat', 'separator' => 'none', 'condition' => ['query_filter' => 'date', 'querydate_mode!' => 'future', 'querydate_field' => 'post_meta']]);
        $this->add_control('querydate_field_meta_future', ['label' => esc_html__('Meta Field', 'dynamic-content-for-elementor'), 'type' => 'ooo_query', 'placeholder' => esc_html__('Meta key or Name', 'dynamic-content-for-elementor'), 'label_block' => \true, 'query_type' => 'metas', 'object_type' => 'post', 'description' => esc_html__('Select a date meta field.', 'dynamic-content-for-elementor'), 'separator' => 'before', 'dynamic' => ['active' => \false], 'condition' => ['query_filter' => 'date', 'querydate_mode' => 'future']]);
        $this->add_control('querydate_field_meta_future_format', ['label' => esc_html__('Meta Date Format', 'dynamic-content-for-elementor'), 'type' => Controls_Manager::TEXT, 'placeholder' => esc_html__('Ymd, Y-m-d, Y-m-d H:i:s, etc.', 'dynamic-content-for-elementor'), 'label_block' => \true, 'default' => '', 'description' => esc_html__('PHP date format.', 'dynamic-content-for-elementor'), 'condition' => ['query_filter' => 'date', 'querydate_mode' => 'future']]);
        $this->add_control('querydate_field_meta_future_format_autodetect', ['type' => Controls_Manager::BUTTON, 'text' => esc_html__('Auto-Detect Date Format', 'dynamic-content-for-elementor'), 'button_type' => 'default', 'event' => 'dce:detectDateFormatFuture', 'separator' => 'none', 'condition' => ['query_filter' => 'date', 'querydate_mode' => 'future']]);
        ...
        if ($settings['query_type'] == 'search_page') {
            ...
            if (!empty($taxonomy_list)) {
                ...
                foreach ($taxonomy_list as $tax) {
                    ...
                    } elseif (is_archive() && isset($queried_object) && $queried_object instanceof \WP_Term) {
            ...
            $args = \array_merge($args, ['post_type' => !empty($settings['post_type']) ? $settings['post_type'] : 'any', 'posts_per_page' => $settings['num_posts'], 'post_status' => 'publish', 'post__in' => $relations_ids, 'orderby' => $settings['orderby'], 'order' => $settings['order']]);
            if ('meta_value' === $settings['orderby']) {
                $args['meta_type'] = $settings['meta_type'];
            }
            ...
            if ('dce_wishlist' === $settings['favorites_key']) {
                ...
                // If none of the favorited items resolve to a displayable product,
                // bail out with an empty query. Passing an empty post__in to
                // WP_Query would otherwise be ignored and return the whole catalog.
                if (empty($wishlist)) {
                    $this->set_empty_query();
                    return;
                }
        ...
        if (\is_array($settings['query_filter']) && !empty($settings['query_filter'])) {
            ...
            if (\in_array('date', $settings['query_filter'], \true)) {
                ...
                if ($settings['querydate_mode'] != 'future' && $settings['querydate_field'] == 'post_meta') {
                    ...
                    // Fallback to default if format is empty
                    if (empty($querydate_field_meta_format)) {
                        $querydate_field_meta_format = 'Ymd';
                    }
                }
    ...
    public function force_sticky_posts_at_the_top(\WP_Query $query_p)
    {
        ...
        $posts_per_page = $query_p->query_vars['posts_per_page'] ?? 0;
        if ($posts_per_page > 0) {
            $query_p->posts = \array_slice($query_p->posts, 0, $posts_per_page);
            $query_p->post_count = \count($query_p->posts);
        }
        return $query_p;
    }
    /**
    ...
    protected function check_custom_query($settings)
    {
        ...
        if (\is_string($custom_query)) {
            ...
            if ($evalError && \Elementor\Plugin::$instance->editor->is_edit_mode()) {
                ...
                echo '</strong><br />';
                echo esc_html__('ERROR', 'dynamic-content-for-elementor') . ': ' . esc_html($e->getMessage()), "\n";
            }
        }
        return \false;
        //+exclude_end
