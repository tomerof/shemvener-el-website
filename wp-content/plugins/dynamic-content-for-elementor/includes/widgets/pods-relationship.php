...
class PodsRelationship extends \DynamicContentForElementor\Widgets\WidgetPrototype
{
    ...
    public function run_once()
    {
        ...
        $save_guard->register_unsafe_control($this->get_name(), 'pods_relation_label');
        $save_guard->register_unsafe_control($this->get_name(), 'pods_relation_text');
    }
    /**
        ...
        if (\count($related_posts) > 1 && $settings['pods_relation_format']) {
            ...
            switch ($settings['pods_relation_format']) {
                ...
                case 'ol':
                    ...
                    break;
                case 'grid':
                    echo '<div class="' . esc_attr('dce-view-row grid-page grid-col-md-' . $settings['pods_relation_col'] . ' grid-col-sm-' . $settings['pods_relation_col_tablet'] . ' grid-col-xs-' . $settings['pods_relation_col_mobile']) . '">';
                    break;
                case 'tab':
                    echo '<div class="' . esc_attr('dce-view-tab dce-tab dce-tab-' . $settings['pods_relation_tab']) . '"><ul>';
                    $i = 0;
                ...
                case 'select':
                    ...
                    if ($settings['pods_relation_close'] && $settings['pods_relation_close_label']) {
                        echo '<option value="#dce-view-no-show">' . esc_html($settings['pods_relation_close_label']) . '</option>';
                    }
                    foreach ($labels as $pkey => $alabel) {
                        echo '<option value="#dce-pods-relational-post-' . esc_attr($this->get_id() . '-' . $pkey) . '">' . esc_html($alabel) . '</option>';
                    }
                    ?>
					</select>
        ...
        foreach ($related_posts as $rkey => $arel) {
            ...
            if (\count($related_posts) > 1) {
                switch ($settings['pods_relation_format']) {
                    case 'ul':
                    case 'ol':
                        echo '<li class="' . esc_attr('dce-view-pane dce-pods-relational-post dce-pods-relational-post-' . $post->ID) . '">';
                        break;
                    default:
                        if ($settings['pods_relation_format'] == 'accordion' && $settings['pods_relation_render'] != 'title') {
                            ...
                            echo esc_attr($this->get_id() . '-' . $post->ID);
                        ...
                        $pane_id = 'dce-pods-relational-post-' . $this->get_id() . '-' . $post->ID;
                        $pane_class = 'dce-view-pane dce-' . $settings['pods_relation_format'] . '-pane dce-pods-relational-post dce-pods-relational-post-' . $post->ID . ($settings['pods_relation_format'] == 'grid' ? ' item-page' : '');
            ...
            if ($settings['pods_relation_render'] == 'template' && $settings['pods_relation_template']) {
                ...
                if ($settings['pods_relation_link']) {
                    echo '<a class="dce-pods-relational-post-link" href="' . esc_url((string) get_permalink($post->ID)) . '">';
                }
