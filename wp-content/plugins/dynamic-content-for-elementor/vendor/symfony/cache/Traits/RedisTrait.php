...
use DynamicOOOS\Predis\Command\Redis\UNLINK;
...
use DynamicOOOS\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use DynamicOOOS\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
...
trait RedisTrait
{
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\DynamicOOOS\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            if (isset($params['redis_sentinel']) && !\is_a($class, \DynamicOOOS\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \DynamicOOOS\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
    ...
    protected function doClear(string $namespace)
    {
        ...
        foreach ($hosts as $host) {
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \DynamicOOOS\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, ?object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \DynamicOOOS\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
