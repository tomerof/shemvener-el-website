...
use DynamicOOOS\Masterminds\HTML5;
/**
...
class Dompdf
{
    ...
    public function loadHtml($str, $encoding = null)
    {
        ...
        if ($encodingGiven && !\in_array(\strtoupper($encoding), ["UTF-8", "UTF8"], \true)) {
        ...
        if (isset($documentEncoding) && isset($offset)) {
            if (!\in_array(\strtoupper($documentEncoding), ["UTF-8", "UTF8"], \true)) {
        ...
        if (!$encodingGiven && !\in_array(\strtoupper($fallbackEncoding), ["UTF-8", "UTF8"], \true)) {
        ...
        try {
            ...
            $quirksmode = \false;
            $html5 = new HTML5(["encoding" => "UTF-8", "disable_html_ns" => \true]);
    ...
    private function processHtml()
    {
        ...
        foreach ($stylesheets as $tag) {
            switch (\strtolower($tag->nodeName)) {
                ...
                case "link":
                    if ((\stripos($tag->getAttribute("rel"), "stylesheet") !== \false || \mb_strtolower($tag->getAttribute("type")) === "text/css") && \stripos($tag->getAttribute("rel"), "alternate") === \false) {
    ...
    public function render()
    {
        ...
        if (\DynamicOOOS\Dompdf\Helpers::lengthEqual($canvasWidth, $size[2]) === \false || \DynamicOOOS\Dompdf\Helpers::lengthEqual($canvasHeight, $size[3]) === \false) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
            $this->fontMetrics->setCanvas($this->canvas);
        }
        $canvas = $this->canvas;
        LineBox::reset_float_reflow_limit();
        // FIXME smelly hack
    ...
    public function setPaper($size, string $orientation = "portrait") : self
    {
        ...
        if (\DynamicOOOS\Dompdf\Helpers::lengthEqual($current_size[2], $new_size[2]) === \false || \DynamicOOOS\Dompdf\Helpers::lengthEqual($current_size[3], $new_size[3]) === \false) {
            $this->canvas = CanvasFactory::get_instance($this, $this->paperSize, $this->paperOrientation);
        }
        return $this;
    }
