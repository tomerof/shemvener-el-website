...
use DynamicOOOS\Dompdf\Css\Content\Attr;
use DynamicOOOS\Dompdf\Css\Content\CloseQuote;
use DynamicOOOS\Dompdf\Css\Content\ContentPart;
use DynamicOOOS\Dompdf\Css\Content\Counter;
use DynamicOOOS\Dompdf\Css\Content\Counters;
use DynamicOOOS\Dompdf\Css\Content\NoCloseQuote;
use DynamicOOOS\Dompdf\Css\Content\NoOpenQuote;
use DynamicOOOS\Dompdf\Css\Content\OpenQuote;
use DynamicOOOS\Dompdf\Css\Content\StringPart;
use DynamicOOOS\Dompdf\Css\Content\Url;
...
class Style
{
     ...
     */
    protected static $_inherited = ["azimuth" => \true, "background_image_resolution" => \true, "border_collapse" => \true, "border_spacing" => \true, "caption_side" => \true, "color" => \true, "cursor" => \true, "direction" => \true, "elevation" => \true, "empty_cells" => \true, "font_family" => \true, "font_size" => \true, "font_style" => \true, "font_variant" => \true, "font_weight" => \true, "font" => \true, "image_resolution" => \true, "letter_spacing" => \true, "line_height" => \true, "list_style_image" => \true, "list_style_position" => \true, "list_style_type" => \true, "list_style" => \true, "orphans" => \true, "overflow_wrap" => \true, "pitch_range" => \true, "pitch" => \true, "quotes" => \true, "richness" => \true, "speak_header" => \true, "speak_numeral" => \true, "speak_punctuation" => \true, "speak" => \true, "speech_rate" => \true, "stress" => \true, "text_align" => \true, "text_indent" => \true, "text_transform" => \true, "visibility" => \true, "voice_family" => \true, "volume" => \true, "white_space" => \true, "widows" => \true, "word_break" => \true, "word_spacing" => \true];
    /**
     * @var array<string, string[]>
     */
    ...
    private function parse_var($matches)
    {
        ...
        if (\in_array($variable, $this->_var_stack, \true)) {
            return null;
        }
    ...
    public function merge(Style $style) : void
    {
        foreach ($style->_props as $prop => $val) {
            ...
            if (\array_key_exists($prop, $style->_props_specified)) {
                $this->_props_specified[$prop] = \true;
            }
        }
    ...
    public function set_prop(string $prop, $val, bool $important = \false, bool $clear_dependencies = \true) : void
    {
        ...
        if (isset(self::$_props_shorthand[$prop])) {
            ...
            if ($this->is_custom_property($prop) && !\in_array($prop, $this->_prop_stack, \true)) {
                ...
                foreach ($specified_props as $specified_prop => $specified_value) {
                    if (!$this->is_custom_property($specified_prop) || \strpos($specified_value, "var({$prop}") !== \false) {
                        $this->set_prop($specified_prop, $specified_value, isset($this->_important_props[$specified_prop]), \true);
                        if (isset(self::$_props_shorthand[$specified_prop])) {
                            foreach (self::$_props_shorthand[$specified_prop] as $sub_prop) {
                                if (\array_key_exists($sub_prop, $specified_props)) {
                                    $this->set_prop($sub_prop, $specified_props[$sub_prop], isset($this->_important_props[$sub_prop]), \true);
                                }
                            }
                        }
                    }
    ...
    {
        return $this->_stylesheet->resolve_url($computed, \true);
    }
    /**
    ...
    {
        return $name !== "none" && !\in_array($name, self::CUSTOM_IDENT_FORBIDDEN, \true);
    }
    /**
    ...
    {
        $val = \strtolower($val);
        $parentFontSize = isset($this->parent_style) ? $this->parent_style->__get("font_size") : self::$default_font_size;
        switch ($val) {
            case "xx-small":
            case "x-small":
            case "small":
    ...
    protected function compute_counter_prop(string $value, int $default, bool $sumDuplicates = \false)
    {
        ...
        foreach ($matches as $match) {
            ...
            $counters[$name] = $sumDuplicates ? ($counters[$name] ?? 0) + $value : $value;
        }
        return $counters;
    }
    /**
    ...
    {
        return $this->compute_counter_prop($val, 1, \true);
    }
    /**
    ...
    protected function _compute_content(string $val)
    {
        ...
        foreach ($components as $value) {
            ...
            if ($function === "attr") {
                ...
                if (!$this->isValidCounterName($name) || !$this->isValidCounterStyleName($type)) {
                    return null;
                }
