    ...
    protected function o_catalog($id, $action, $options = '')
    {
        ...
        switch ($action) {
            ...
            case 'outputIntents':
                ...
                $this->o_contents($this->numObj, 'add', ['N' => $options['colorComponentsCount']]);
                $o['info']['outputIntents'][] = ['iccProfileName' => $options['iccProfileName'], 'destOutputProfile' => $this->numObj];
                break;
            ...
            case 'metadata':
                ...
                $this->o_contents($this->numObj, 'add', ['Type' => '/Metadata', 'Subtype' => '/XML']);
                break;
            case 'out':
                $res = "\n{$id} 0 obj\n<< /Type /Catalog";
                foreach ($o['info'] as $k => $v) {
...
EOT;
        ...
        switch ($action) {
            ...
            case 'out':
                ...
                if ($this->pdfa) {
                    ...
                    if (isset($pagesInfo['fonts']) && \count($pagesInfo['fonts']) || isset($pagesInfo['procset']) || isset($pagesInfo['extGStates']) && \count($pagesInfo['extGStates'])) {
                        $res .= "\n/Resources <<";
    ...
    public function enablePdfACompliance()
    {
        ...
        $this->o_catalog($this->catalogId, 'outputIntents', ['iccProfileData' => \file_get_contents($iccProfilePath), 'iccProfileName' => \basename($iccProfilePath), 'colorComponentsCount' => '3']);
    }
...
EOT;
    ...
    {
        static $blendModes = ["Normal", "Multiply", "Screen", "Overlay", "Darken", "Lighten", "ColorDogde", "ColorBurn", "HardLight", "SoftLight", "Difference", "Exclusion"];
        if (!\in_array($mode, $blendModes, \true)) {
            $mode = "Normal";
        }
        $newState = ["mode" => $mode, "opacity" => $opacity];
    ...
    function filterName($text)
    {
        ...
        while (isset($char_array[++$char_index])) {
            ...
            $dec = \ord($char);
            if ($dec >= 33 && $dec <= 126 && !\in_array($char, $delimeters, \true)) {
                $name .= $char;
            } else {
    ...
    public function addEmbeddedFile(string $filepath, string $embeddedFilename, string $description, string $mimeType = "application/octet-stream", array $relatedTo = []) : void
    {
        ...
        $this->o_embedded_file_dictionary($efDictId, 'new', ['filepath' => $filepath, 'filename' => $embeddedFilename, 'description' => $description, 'mimeType' => $mimeType]);
        if ($relatedTo !== []) {
    ...
    function addSvgFromFile($file, $x, $y, $w = 0, $h = 0)
    {
        ...
        $surface = new \DynamicOOOS\Svg\Surface\SurfaceCpdf($doc, $this);
        $doc->render($surface);
        $this->restore();
    }
    /**
