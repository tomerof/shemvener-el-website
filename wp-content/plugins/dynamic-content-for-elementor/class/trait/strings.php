...
trait Strings
{
    ...
    //+exclude_start
    /**
     * Convert an Elementor div-based layout into a table-based layout for HTML emails.
     *
     * Elements carrying the `elementor-container`, `elementor-row` and `elementor-column`
     * classes are re-tagged to `table`, `tr` and `td` respectively, keeping their
     * attributes and inner content. Uses the native DOM extension, so it pulls in no
     * external HTML-parsing dependency.
     *
     * LEGACY: this only handles the classic Elementor Section markup. `.elementor-row`
     * is already obsolete (removed by Elementor's Optimized DOM Output) and the Flexbox
     * Container element (`.e-con` / `.e-con-inner`, the default since Elementor 3.6) is
     * not converted at all. Kept working only for email templates built with Sections.
     *
     * @param string $html Source HTML fragment.
     * @return string Transformed HTML fragment.
     */
    public static function tablefy($html = '')
    {
        ...
        // The XML hint forces UTF-8 parsing; the explicit <body> keeps the fragment
        // intact (a stray closing tag has no added wrapper of ours to escape from).
        ...
        // Preserve the previous parser's cleanup: drop scripts, styles and comments.
        $to_remove = $xpath->query('//script | //style | //comment()');
        if (\false !== $to_remove) {
            foreach (\iterator_to_array($to_remove) as $node) {
                if ($node instanceof \DOMNode && null !== $node->parentNode) {
                    $node->parentNode->removeChild($node);
                }
            }
        }
        // The DOM extension cannot rename a node in place, so build a replacement
        // element, move the attributes and children over, then swap it in.
        $rename = function (\DOMElement $node, $new_name) use($doc) {
            $new = $doc->createElement($new_name);
            if (\false === $new || null === $node->parentNode) {
                return;
            }
            foreach (\iterator_to_array($node->attributes) as $attr) {
                $new->setAttributeNode($attr->cloneNode(\true));
            }
        ...
        // Classic Section classes only; Flexbox Container markup (.e-con) is not handled.
        $map = ['elementor-container' => 'table', 'elementor-row' => 'tr', 'elementor-column' => 'td'];
        foreach ($map as $class => $tag) {
            // Whole-word class match, mirroring the CSS `.class` selector used before.
