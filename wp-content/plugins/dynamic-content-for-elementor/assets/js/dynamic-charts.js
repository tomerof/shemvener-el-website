(function ($) {
	var WidgetElements_DynamicCharts = function ($scope, $) {
		...
		// Animation
		if (elementSettings.animation_enabled) {
			options.animation = {
				duration: elementSettings.chart_animation_duration || 1000,
				easing: elementSettings.animation_easing || "easeOutQuart",
			};
		} else {
		...
		let datasetOptions = {
			...
			data: data,
			fill: elementSettings.fill_area ? 'origin' : false,
			backgroundColor: backgroundColors,
			borderColor: borderColors,
			borderWidth: elementSettings.border_width_data || 0,
		};

		if ("line" === elementSettings.type) {
			datasetOptions.tension = elementSettings.line_tension?.size || 0;
			datasetOptions.pointStyle = elementSettings.point_style === 'false' ? false : (elementSettings.point_style || 'circle');
			datasetOptions.pointRadius = elementSettings.point_radius || 3;
		}

		if ("bar" === elementSettings.type) {
			datasetOptions.borderRadius = elementSettings.bar_border_radius || 0;
		}
