...
/**
 * Resolve the effective value of a responsive control for the current device,
 * honoring Elementor's active breakpoints (custom ones included) and inheritance.
 *
 * @param {Object} elementorSettings The element settings object.
 * @param {string} key The base control key (e.g. 'size_honeycombs').
 * @param {*} defaultValue Fallback used when nothing is set.
 * @return {*} The value that applies on the current device mode.
 */
dynamicooo.getResponsiveValue = (elementorSettings, key, defaultValue) => {
	const breakpoints = dynamicooo.getActiveBreakpointsMinPointAndPostfix();
	const currentMode = elementorFrontend.getCurrentDeviceMode();
	const currentMin = breakpoints[currentMode]
		? breakpoints[currentMode].min_point
		: 0;
	let value =
		elementorSettings[key] !== undefined &&
		elementorSettings[key] !== null &&
		elementorSettings[key] !== ""
			? elementorSettings[key]
			: defaultValue;
	Object.keys(breakpoints)
		.sort((a, b) => breakpoints[b].min_point - breakpoints[a].min_point)
		.forEach((name) => {
			if (breakpoints[name].min_point < currentMin) {
				return;
			}
			const v = elementorSettings[key + breakpoints[name].postfix];
