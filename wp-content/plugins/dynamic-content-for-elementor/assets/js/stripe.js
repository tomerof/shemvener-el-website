...
function makeStripeRequestFingerprint(url, data) {
	const values = [];
	...
	return JSON.stringify([url, values]);
}
...
function initializeStripeField(wrapper, $scope) {
	...
	const stripeNonce = $elementsWrapper.attr("data-stripe-nonce");
	...
	const fetchClientSecret = () => {
		...
		data.set("nonce", stripeNonce);
		const fingerprint = makeStripeRequestFingerprint(url, data);
		if (!requestId || requestFingerprint !== fingerprint) {
			requestId = makeStripeRequestId();
