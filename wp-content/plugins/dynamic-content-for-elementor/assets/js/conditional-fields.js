...
const getOnFormChange = ({
	...
	const deactivateField = (id, disableOnly) => {
		...
		const isFileInput = $fieldInputs.length === 1 && $fieldInputs.attr("type") === "file";
		if (isFileInput) {
			// For file inputs, don't disable - we'll clear them on submit instead.
			// Disabling would prevent the field from being submitted at all,
			// causing Elementor's validation to fail with a count() error on NULL.
			// But we must remove 'required' to prevent browser validation error on hidden field.
			if ($fieldInputs.prop("required")) {
				$fieldInputs.data("dce-was-required", true);
				$fieldInputs.prop("required", false);
			}
		} else {
	...
	const activateField = (id, disableOnly) => {
		...
		// Restore required attribute for file inputs if it was removed
		if (isFileInput && $fieldInputs.data("dce-was-required")) {
			$fieldInputs.prop("required", true);
		}
		const wasActive = $fieldWrapper[0].dataset.dceConditionsFieldStatus === "active";
		...
		if ($fieldInputs.data("hide") !== "yes") {
			$fieldWrapper.show();
			// Trigger custom event only on transition to visible, not on every form change
			if (!wasActive) {
...
function initializeConditionalFields($form) {
	...
	// Clear hidden file inputs just before form submission to prevent validation errors.
	// We do this on submit rather than when hiding to preserve the file selection
	// in case the field becomes visible again before submission.
	// Using capturing phase (true) ensures this runs before Elementor's bubbling handlers.
	...
	if (formElement) {
		formElement.addEventListener(
			...
			function () {
				$form
					.find('.elementor-field-group[data-dce-conditions-field-status="inactive"] input[type="file"]')
					.val("");
			},
			true,
		);
